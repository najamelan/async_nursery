...
#[ derive( Debug ) ]
//
pub struct LocalNursery<S, Out> where S: Unpin, Out: 'static
{
	spawner     : S                                                  ,
...
impl<S, Out> LocalNursery<S, Out> where S: Unpin + LocalSpawnHandle<Out> + LocalSpawnHandle<()>, Out: 'static
{
	...
	/// get an error.
	//
	pub fn handle( &mut self ) -> LocalNurseryHandle<S, Out> where S: Clone
	{
 		// We should always have a tx, so unwrap should be fine.
 		//
 		let tx = self.tx.clone();

		LocalNurseryHandle::new( self.spawner.clone(), tx, self.in_flight.clone(), self.closed.clone() )
	}
...
impl<S> LocalSpawn for LocalNursery<S, ()> where S: Unpin + LocalSpawnHandle<()>
{
...
impl<S, Out> Stream for LocalNursery<S, Out>

	where S: Unpin, Out: 'static
{
	type Item = Out;
		...
		match poll_stream
		{
			Poll::Ready( None ) =>
			{
				...
				else
				{
					...
					debug!( "this.in_flight: {}", this.in_flight.load( SeqCst ) );
					debug!( "this.closed: {}"   , this.closed   .load( SeqCst ) );
...
impl<S, Out> Sink<LocalFutureObj<'static, Out>> for LocalNursery<S, Out>
